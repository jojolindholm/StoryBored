...
type CopyToClipboardOptions = {
  ...
  /** optional SVG payload to include for interoperability */
  svg?: string | null;
};
...
export const copyToClipboard = async (
  ...
  const clipboardJSON = serializeAsClipboardJSON({ elements, files });
  const { clipboardEvent, svg } = options;

  if (clipboardEvent) {
    try {
      clipboardEvent.clipboardData?.setData(MIME_TYPES.text, clipboardJSON);
      clipboardEvent.clipboardData?.setData(MIME_TYPES.json, clipboardJSON);
      if (svg) {
        clipboardEvent.clipboardData?.setData(MIME_TYPES.svg, svg);
        clipboardEvent.clipboardData?.setData(MIME_TYPES.html, svg);
      }
      return;
    } catch (error: any) {
      console.error(error);
  ...
  if (navigator.clipboard?.write && "ClipboardItem" in window) {
    try {
      const itemData: Record<string, Blob | Promise<Blob>> = {
        [MIME_TYPES.text]: new Blob([clipboardJSON], {
          type: MIME_TYPES.text,
        }),
        [MIME_TYPES.json]: new Blob([clipboardJSON], {
          type: MIME_TYPES.json,
        }),
      };
      if (svg) {
        itemData[MIME_TYPES.svg] = new Blob([svg], { type: MIME_TYPES.svg });
        itemData[MIME_TYPES.html] = new Blob([svg], { type: MIME_TYPES.html });
      }

      await navigator.clipboard.write([
        new window.ClipboardItem(itemData as Record<string, Blob | Promise<Blob>>),
      ]);
      return;
    } catch (error: any) {
      console.error(error);
  ...
  await copyTextToSystemClipboard(clipboardJSON, clipboardEvent);
};
